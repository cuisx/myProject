...
var LINK_TYPES = {
  phone: {
    ...
    transform: function phoneTransform(phone, link) {
      // Added by guirw@tcl.com for extra (.
      var phone_back = phone;
      if (phone.indexOf('(') == 0) {
        phone_back = phone.substring(1, phone.length);
      }
      // Added end.
      return '<a dir="ltr" data-dial="' + phone_back +
        '" data-action="dial-link" class="navigable">' + phone + '</a>';
    }
  },
  ...
  url: {
    ...
    transform: function urlTransform(url, linkSpec) {
      ...
      return '<a data-url="' + href + '" data-action="url-link" class="navigable">' + url +
             '</a>';
    }
  },

  // Added by guirw@tcl.com for test case 4931 for add URI.
  uri: {
    regexp: new RegExp([
      // must begin at start of string, after whitespace,
      // {1} match the protocol https?:// or rtsp:// or midlet:// or ams:// or tel:// or device:// or wtai:// (optional)
      '(https?://|rtsp://|midlet://|ams://|tel://|device://|wtai://)',
      // {2} match "server name": . must be followed by at least one letter
      '((?:\\.?[-\\w]){1,256})',
      // optional :port
      '(?::[0-9]{1,5})?',
    ...
    transform: function urlTransform(url, linkSpec) {
      ...
      if (href.toLowerCase().indexOf('tel://') != -1) { // we need mark tel:// as a phone number.
        href = href.substring(6, href.length);
        return '<a dir="ltr" data-dial="' + href + '" data-action="dial-link" class="navigable">' + url +
               '</a>';
      } else if (href.toLowerCase().indexOf('wtai://') != -1) { // we need mark wtai:// as a phone number.
  ...
  email: {
    regexp: /[\w.+-]+@[\w.-]+\.[a-z]{2,6}/mgi, // TODO We should not mark . as email at last. Modified by guirw@tcl.com.
    transform: function emailTransform(email) {
      return [
        '<a data-email="',
        '" data-action="email-link" class="navigable">',
        '</a>'
      ].join(email);
    }
  }
